...
namespace llvm {
//===----------------------------------------------------------------------===//
// SlotTracker_ Class: Enumerate slot numbers for unnamed values
//===----------------------------------------------------------------------===//
...
public:
  ...
  explicit SlotTracker_(const Module *M,
  ...
  /// functions, giving correct numbering for metadata referenced only from
  /// within a function (even if no functions have been initialized).
  explicit SlotTracker_(const Function *F,
                       bool ShouldInitializeAllMetadata = false);

  /// Return the slot number of the specified value in it's type
  /// plane.  If something is not in the SlotTracker_, return -1.
  ...
  /// this method to get its data into the SlotTracker_.
  ...
  /// most recently incorporated function from the SlotTracker_. This
...
private:
  ...
  SlotTracker_(const SlotTracker_ &) = delete;
  void operator=(const SlotTracker_ &) = delete;
};
} // namespace llvm

static SlotTracker_ *createSlotTracker_(const Value *V) {
  if (const Argument *FA = dyn_cast<Argument>(V))
    return new SlotTracker_(FA->getParent());

  if (const Instruction *I = dyn_cast<Instruction>(V))
    if (I->getParent())
      return new SlotTracker_(I->getParent()->getParent());

  if (const BasicBlock *BB = dyn_cast<BasicBlock>(V))
    return new SlotTracker_(BB->getParent());

  if (const GlobalVariable *GV = dyn_cast<GlobalVariable>(V))
    return new SlotTracker_(GV->getParent());

  if (const GlobalAlias *GA = dyn_cast<GlobalAlias>(V))
    return new SlotTracker_(GA->getParent());

  if (const GlobalIFunc *GIF = dyn_cast<GlobalIFunc>(V))
    return new SlotTracker_(GIF->getParent());

  if (const Function *Func = dyn_cast<Function>(V))
    return new SlotTracker_(Func);
...
SlotTracker_::SlotTracker_(const Module *M, bool ShouldInitializeAllMetadata)
...
SlotTracker_::SlotTracker_(const Function *F, bool ShouldInitializeAllMetadata)
    : TheModule(F ? F->getParent() : nullptr), TheFunction(F),
      FunctionProcessed(false),
      ShouldInitializeAllMetadata(ShouldInitializeAllMetadata), mNext(0),
      fNext(0), mdnNext(0), asNext(0) {}

inline void SlotTracker_::initialize() {
  if (TheModule) {
    processModule();
...
void SlotTracker_::processModule() {
...
void SlotTracker_::processFunction() {
...
void SlotTracker_::processGlobalObjectMetadata(const GlobalObject &GO) {
...
void SlotTracker_::processFunctionMetadata(const Function &F) {
...
void SlotTracker_::processInstructionMetadata(const Instruction &I) {
...
void SlotTracker_::purgeFunction() {
...
int SlotTracker_::getGlobalSlot(const GlobalValue *V) {
...
int SlotTracker_::getMetadataSlot(const MDNode *N) {
  // Check for uninitialized state and do lazy initialization.
  initialize();
...
int SlotTracker_::getLocalSlot(const Value *V) {
...
int SlotTracker_::getAttributeGroupSlot(AttributeSet AS) {
  // Check for uninitialized state and do lazy initialization.
  initialize();
...
/// CreateModuleSlot - Insert the specified GlobalValue* into the slot table.
void SlotTracker_::CreateModuleSlot(const GlobalValue *V) {
  assert(V && "Can't insert a null Value into SlotTracker_!");
...
void SlotTracker_::CreateFunctionSlot(const Value *V) {
...
void SlotTracker_::CreateMetadataSlot(const MDNode *N) {
  assert(N && "Can't insert a null Value into SlotTracker_!");
...
void SlotTracker_::CreateAttributeSetSlot(AttributeSet AS) {
...
static void WriteAsOperandInternal(raw_ostream &Out, const Value *V,
                                   ...
                                   SlotTracker_ *Machine,
                                   const Module *Context);

static void WriteAsOperandInternal(raw_ostream &Out, const Metadata *MD,
                                   TypePrinting *TypePrinter,
                                   SlotTracker_ *Machine, const Module *Context,
...
static void writeMDTuple(raw_ostream &Out, const MDTuple *Node,
                         TypePrinting *TypePrinter, SlotTracker_ *Machine,
                         const Module *Context) {
  Out << "!{";
...
static void writeDISubrange(raw_ostream &Out, const DISubrange *N,
                            TypePrinting *, SlotTracker_ *, const Module *) {
  Out << "!DISubrange(";
...
static void WriteAsOperandInternal(raw_ostream &Out, const Value *V,
  ...
  if (Machine) {
    if (const GlobalValue *GV = dyn_cast<GlobalValue>(V)) {
      ...
      // address of blocks.
      if (Slot == -1)
        if ((Machine = createSlotTracker_(V))) {
          Slot = Machine->getLocalSlot(V);
          delete Machine;
        }
    }
  } else if ((Machine = createSlotTracker_(V))) {
...
static void WriteAsOperandInternal(raw_ostream &Out, const Metadata *MD,
  ...
  if (const MDNode *N = dyn_cast<MDNode>(MD)) {
    std::unique_ptr<SlotTracker_> MachineStorage;
    if (!Machine) {
      MachineStorage = make_unique<SlotTracker_>(Context);
...
class AssemblyWriter {
  ...
  const Module *TheModule;
  std::unique_ptr<SlotTracker_> SlotTracker_Storage;
  SlotTracker_ &Machine;
  TypePrinting TypePrinter;
...
public:
  /// Construct an AssemblyWriter with an external SlotTracker_
  AssemblyWriter(formatted_raw_ostream &o, SlotTracker_ &Mac, const Module *M,
...
AssemblyWriter::AssemblyWriter(formatted_raw_ostream &o, SlotTracker_ &Mac,
...
void AssemblyWriter::writeAllMDNodes() {
  ...
  for (SlotTracker_::mdn_iterator I = Machine.mdn_begin(), E = Machine.mdn_end();
       I != E; ++I)
...
void AssemblyWriter::writeAllAttributeGroups() {
  ...
  for (SlotTracker_::as_iterator I = Machine.as_begin(), E = Machine.as_end();
       I != E; ++I)
    asVec[I->second] = *I;
...
int main(int argc, char *argv[]) {
  ...
  ///*ErrorOr*/Expected<llvm::Module *> moduleOrErr =
  ...
  if(moduleOrErr.takeError()){// this "if" is needed
